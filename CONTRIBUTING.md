...
```sh
find . \( -name "*.py" -o -name "*.md" -o -name "*.txt" \) -type f -print0 | while IFS= read -r -d '' file; do echo "--- Conteúdo do arquivo: $file ---" >> todos-arquivos.txt; cat "$file" >> todos-arquivos.txt; echo -e "\n" >> todos-arquivos.log; done
```